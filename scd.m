classdef scd
    ...
    %%%%KF edits 3/20/2017%%%%%
    %scd was edited in several ways. To start, we now will export
    %separation distance and mahal distance as part of the fcs files for
    %the debarcoded samples, allowing users to gate outside of the
    %debarcoder for stringency.
    %2nd modification: the default mahal value will now be automatically
    %adjusted to be the max mahal distance of the imported fcs files, and
    %the color bar will be updated accordingly
    %%%end KF edits 3/20/2017%%%%
    ...
    methods
        ...
        function obj = scd(key_filename)
            ...
            %%%% KF edits 3/20/2017%%%%%
            %Changed defaults to 0 and inf
            
            % set defaults
            obj.default_cofactor=10;
            obj.sep_cutoff=0;
            obj.mahal_cutoff_val=Inf;
...
%             when loaded
        ...
        function obj = compute_mahal(obj)
            ...
            %%%% KF edits 3/20/2017%%%%%%%%
            % after calculating all of the mahal distances for all of the
            % samples with mahal set to infinity, we reset the mahal cutoff
            % value that the user sees to be the maximum mahal distance in
            % the samples. The +1 is so the color scale bar will include
            % the sample with the highest mahal distance
            
            obj.mahal_cutoff_val=max(max(obj.mahal))+1; %reset mahal cutoff to the max mahal value +1
            %%%% end KF edits %%%%%%%%%%%%
        end
        ...
        function write_bc_fcs_files(obj,outdir,basename)
            ...
            %%%%%%%%%%%% BEGIN EDITS EZ&KS, added 3/20/2017 by KF %%%%%%%%%%%%%%
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                	size(obj.x)
%                   size(obj.x(thiswell_bin,:))
%                   size(obj.deltas)
%                   size(obj.mahal)
%                   whos obj.deltas
%                   whos obj.mahal
                    alldat = [obj.x, obj.deltas, obj.mahal];
                    newm={obj.m{:},'bc_separation_dist','mahalanobis_dist'};
                    newc={obj.c{:},'bc_separation_dist','mahalanobis_dist'};
%                        datamatrix=[obj.x(thiswell_bin,:),obj.deltas,obj.mahal]

                data=alldat(thiswell_bin,:);
                if ~isempty(data)
                    fca_writefcs_op([outdir filesep basename '_' obj.wellLabels{i} '.fcs'],data,newm,newc); %obj.m,obj.c)
                end
            end
            ...
            newm={obj.m{:},'bc_separation_dist','mahalanobis_dist'};
            newc={obj.c{:},'bc_separation_dist','mahalanobis_dist'};         
            data=alldat(not_inawell,:);
               
            fca_writefcs_op([outdir filesep basename '_unassigned.fcs'],data,newm,newc);%obj.m,obj.c)            
